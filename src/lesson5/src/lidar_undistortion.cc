...
LidarUndistortion::LidarUndistortion() : private_node_("~")
{
    ...
    if (use_imu_)
    {
        imu_subscriber_ = node_handle_.subscribe(
            "IMU_data", 2000, &LidarUndistortion::ImuCallback, this, ros::TransportHints().tcpNoDelay());
    }
    ...
    laser_scan_subscriber_ = node_handle_.subscribe(
        "scan", 5, &LidarUndistortion::ScanCallback, this, ros::TransportHints().tcpNoDelay());
